...
import logging
import time
from retro_star.common import prepare_starting_molecules, prepare_mlp, prepare_molstar_planner_fn, prepare_molstar_planner, smiles_to_fp
...
class value_fnc():
    def __init__(self, model, fp_dim, device):
        self.model = model
        self.fp_dim = fp_dim
        self.device = device
...
class RSPlanner:
    def __init__(self,
        ...
        if use_value_fn:
            ...
            model.eval()
            value_fn = value_fnc(model, fp_dim, device)

        else:
            value_fn = lambda x: 0.

        self.plan_handle = prepare_molstar_planner_fn(
            one_step=one_step,
            value_fn=value_fn,
